...
export default async function ActivityPage({
		...
		return (
			<ActivityMotion>
				<div className="max-w-[1000px] m-auto flex justify-center flex-col items-center my-12 gap-3">
					...
					<span className="text-slate-500 text-center">
						We&apos;re sorry. The activity you&apos;re looking for wasn&apos;t found or
						was deleted.
					</span>
					<Button variant="ghost" className="w-full">
						<Link href="/app/explore">Go to feed</Link>
	...
	return (
		<section className="w-full m-auto flex flex-col h-full max-w-[900px] ">
			...
			<div className="flex items-center gap-2">
				...
				<div className="">
					<p className="text-darkGray font-medium text-sm">
						{activity[0].users?.displayName && activity[0].users?.displayName !== ''
			...
			<p className="text-slate-500 text-sm mb-6 overflow-hidden break-words	text-ellipsis">
				{activity[0].description}
			</p>
			<Badge variant="secondary" className="w-fit">
				{getCategoryNameById(activity[0].category_id)}
			</Badge>
			<section className="mr-auto max-w-full">
					...
					<Card className="lg:max-w-[300px] w-full">
						...
						<CardContent className="">
							{userId ? (
								...
								<span className="text-sm font-medium">You cannot rate as a guest.</span>
							)}
						</CardContent>
					</Card>
				</div>
